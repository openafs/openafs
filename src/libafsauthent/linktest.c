...
{
    int code;
    struct afsconf_cell cellinfo[1];
    struct afsconf_dir *adir = 0;
    char pass[512], *reason;
    ...
    if (code == 666) {
	code = afsconf_GetNoAuthFlag(adir);
	PrintEntry(0, 0, 0);
	ka_UserReadPassword("Password", pass, code, &reason);
	code = afsconf_SetCellInfo(adir, pass, cellinfo);
	ka_PrintUserID(0, 0, 0, 0);
	code = ugen_ClientInit(code, pass, reason, 0, 0, 0, 0,
	    0, 1, pass, 50, 0, 0, 0);
    }
    if (code) {
#ifdef AFS_RXK5
