...
RCSID("$Header: /tmp/cvstemp/openafs/src/uss/uss_kauth.c,v 1.1.1.6 2003/04/13 19:08:02 hartmans Exp $");
...
{ /*InitThisModule*/
     ...
     */
    code = ka_GetAdminToken(0, 0, uss_Cell, 0, 10*60*60, &token, 0/*new*/);
    if (code) {
	if (Pipe) {
	    ...
	    strcat(prompt, "': ");
	    code =
		ka_UserReadPassword(prompt,		  /*Prompt to use*/
	...
	code = ka_GetAdminToken(uss_AccountCreator,
				CreatorInstance,
				uss_Cell,
				&key,
				24*60*60,
	...
	if (code) {
		...
		if (code) {
		     ...
		     */
		    printf("%s: Shortened password accepted by the Authentication Server\n", uss_whoami);
		}
     ...
     */
    code = ka_AuthServerConn(uss_Cell, KA_MAINTENANCE_SERVICE,
			     &token, &uconn_kauthP);
    if (code) {
	com_err(uss_whoami, code,
	...
	if (code = ktc_GetToken (&Name, &token, sizeof(struct ktc_token), &tok)) {
	    code = ka_UserAuthenticateLife (0, uss_AccountCreator, CreatorInstance,
					    uss_Cell,longPassBuff,10*60*60,&reasonString);
	    if (!code)
		doUnlog = 1;
	}
    }
...
afs_int32 uss_kauth_AddUser(a_user, a_passwd)
    char *a_user;
    char *a_passwd;
...
{ /*uss_kauth_AddUser*/
	 ...
	 */
	if (uss_verbose)
	    printf("[Skip Kaserver option - Adding of user %s in Authentication DB not done]\n",
		   a_user);
	return 0;
    }
...
afs_int32 uss_kauth_DelUser(a_user)
    char *a_user;
...
{ /*uss_kauth_DelUser*/
	 ...
	 */
	if (uss_verbose)
	    printf("[Skip Kaserver option - Deleting of user %s in Authentication DB not done]\n",
		   a_user);
	return 0;
    }
	...
	if (code) {
	    if (code == KANOENT) {
		if (uss_verbose)
		    printf("%s: No entry for user '%s' in Authentication DB\n",
			   uss_whoami, a_user);
		return(0);
	    }
	    else {
...
afs_int32 uss_kauth_CheckUserName()
...
{ /*uss_kauth_CheckUserName*/
     ...
     */
    code = ka_ParseLoginName(uss_User,
			     UserPrincipal, UserInstance, UserCell);
...
afs_int32 uss_kauth_SetFields(username, expirestring, reuse, failures, lockout)
    char *reuse;
    char *username;
    char *expirestring;
    char *failures;
...
{ 
	...
	if (was_spare || flags || expiration || 
	    lifetime || (maxAssociates >= 0)) {
	...
	if (code) {
	  com_err (uss_whoami, code,
		   "calling KAM_SetFields for %s.%s", username, instance);
