...
static int CMtoUNIXerror(int cm_code)
{
	switch (cm_code) {
		case CM_ERROR_TIMEDOUT: return ETIMEDOUT;
		case CM_ERROR_NOACCESS: return EACCES;
		case CM_ERROR_NOSUCHFILE: return ENOENT;
		case CM_ERROR_INVAL: return EINVAL;
		case CM_ERROR_BADFD: return EBADF;
		case CM_ERROR_EXISTS: return EEXIST;
		case CM_ERROR_CROSSDEVLINK: return EXDEV;
		case CM_ERROR_NOTDIR: return ENOTDIR;
		case CM_ERROR_ISDIR: return EISDIR;
		case CM_ERROR_READONLY: return EROFS;
		case CM_ERROR_WOULDBLOCK: return EWOULDBLOCK;
		case CM_ERROR_NOSUCHCELL: return ESRCH;		/* hack */
		case CM_ERROR_NOSUCHVOLUME: return EPIPE;	/* hack */
		case CM_ERROR_NOMORETOKENS: return EDOM;	/* hack */
		case CM_ERROR_TOOMANYBUFS: return EFBIG;	/* hack */
		default: return ENOTTY;
	}
}

static void InitFSRequest(fs_ioctlRequest_t *rp)
{
	rp->mp = rp->data;
        rp->nbytes = 0;
}

static long GetIoctlHandle(char *fileNamep, HANDLE *handlep)
{
	char *drivep;
	char hostName[256];
        char tbuffer[100];
...
static long Transceive(HANDLE handle, fs_ioctlRequest_t *reqp)
{
	long rcount;
        long ioCount;

        rcount = reqp->mp - reqp->data;
	if (rcount <= 0) return EINVAL;		/* not supposed to happen */
...
static long MarshallLong(fs_ioctlRequest_t *reqp, long val)
{
...
static long MarshallString(fs_ioctlRequest_t *reqp, char *stringp)
{
	int count;
	...
	/* watch for buffer overflow */
        if ((reqp->mp - reqp->data) + count > sizeof(reqp->data)) return -1;
        
	if (stringp) memcpy(reqp->mp, stringp, count);
	else *(reqp->mp) = 0;
        reqp->mp += count;
        return 0;
}
 ...
 */
static long fs_GetFullPath(char *pathp, char *outPathp, long outSize)
{
	char tpath[1000];
	char origPath[1000];
        char *firstp;
...
long pioctl(char *pathp, long opcode, struct ViceIoctl *blobp, int follow)
{
	...
	/* now make the call */
        ...
        if (blobp->out_size) {
		temp = blobp->out_size;
                if (preq.nbytes < temp) temp = preq.nbytes;
