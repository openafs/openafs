...
clientHandle_t *getClientHandle()
{
    int i;
...
/*    lwp_debug = 1; */
    ...
    for (i=0; i<MAX_THREADS; i++) {
	if (LWP_CreateProcess(handleRequest, 32768,
			      LWP_NORMAL_PRIORITY,
			      (char*)&clientHandles[i],
			      "HandleRequestThread",
			      &pid) < 0 ) {
	...
	FD_SET(sockFD, rfds); FD_SET(sockFD, efds);

	Log("Main - going to select.\n");
	code = IOMGR_Select(sockFD+1, rfds, wfds, efds, (struct timeval*)0);
	switch(code) {
	...
	default:
		...
		clientHandle->ch_fd = accept(sockFD,
				     (struct sockaddr*)&clientHandle->ch_addr,
					  &addr_len);
...
void handleRequest(char *arg)
{
	...
	code = IOMGR_Select(ch->ch_fd + 1, &(ch->ch_read), &(ch->ch_write),
...
int write_I = 0;
void handleWrite(clientHandle_t *ch, selcmd_t *sc)
{
    int i;
    fd_set *rfds, *wfds, *efds;
    char *buf;
    ...
    while ( 1 ) {
	FD_ZERO(rfds); FD_SET(ch->ch_fd, rfds);
	FD_ZERO(efds); FD_SET(ch->ch_fd, efds);
	FD_ZERO(wfds);
	scode = IOMGR_Select(ch->ch_fd+1, rfds, wfds, efds,
			   (struct timeval*)0);
	assert(scode>0);
