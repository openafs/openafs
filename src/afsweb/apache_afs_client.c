 ...
 */
static int parse_authhdr(request_rec *r, 
		  char *user,
		  char *passwd,
		  char *cell,
		  char *defaultCell)
{
   ...
   */
  if (strncasecmp(GETWORD(r->pool,(const char **)&auth_line,' '),
		  "basic",6) != 0) {
    /* Client tried to authenticate using some other auth scheme */
    LOG_REASON ("AFSAUTH_CLIENT:client used other than Basic authentication" 
		"scheme", r->uri, r);
    return SERVER_ERROR;
  }
 ...
 */
static int sendBuffer(char *buf, int len)
{
  afsassert(buf);
  if (write(writePipe, buf, len) != len) { 
    afslog(5, ("%s: Error writing to pipe - %s", module_name, strerror(errno)));
    return -1;
  }
  return 0;
}
 ...
 */
static int sendTo_afsAuthenticator(char *user, char *passwd, 
				   char *cell, char *type) 
{
  char buf[MAXBUFF];
 ...
 */
static int recvFrom_afsAuthenticator(char *buf)
{
  int n;
  ...
  if(n < 0) {
    afslog(5, ("%s: Error reading from pipe - %s", module_name, strerror(errno)));
    return -1;
  }
  return n;
}
 ...
 */
static int check_Cache(char *user, char *passwd, char *cell, char *tokenBuf)
{
 ...
 */
static int updateCache(char *user, 
		char *passwd, 
		char *cell, 
		char *tokenBuf,
		int cacheExpiration)
{
  ...
  if (expires < 0 ) {
    afslog(5, ("%s: Error getting expiration time for cache. Expires %d",
	       module_name, expires));
    return -1;
  }
  ...
  if (cacheExpiration == 0) {
    weblog_login_store(user, cell, cksum, &tokenBuf[0], sizeof(tokenBuf),expires);
  }
  else {
 ...
 */
static int start_lock(int fd, int cmd, int type)
{
  struct flock lock;
  lock.l_type=type;
  lock.l_start = 0;
...
static int test_lock(int fd, int type)
{
  struct flock lock;
  lock.l_type=type;
  lock.l_start = 0;
 ...
 */
static int request_Authentication(char *user, char *passwd, 
				  char *cell, char *type,
 ...
 */
  ...
  while ((pid = Is_writelock(lockfd)) != 0) {
    if (pid < 0) {
      afslog(5, ("%s: pid:%d Error locking pipe - %s", 
		 module_name, getpid(), strerror(errno)));
      return -1;
    }
    afslog(40, ("%s: pid %d waiting for lock held by pid %d",
		module_name, getpid(), pid));
  }

  if(Write_lock(lockfd) == -1) {
    afslog(5, ("%s: pid:%d Error write lock - %s. Retrying with WriteW", 
	       module_name, getpid(), strerror(errno)));
    if (Writew_lock(lockfd) == -1) {
      afslog(5, ("%s: pid:%d Error write lock - %s", 
		 module_name, getpid(), strerror(errno)));
      return -1;
    }
  }
 ...
 */
static int setToken(char *tokenBuf, int tokenLen)
{
  register char *temp;
  afs_int32 i=0;
  ...
  return do_pioctl(tokenBuf, tokenLen, tokenBuf, tokenLen, VIOCSETTOK, NULL, 0);
}
...
static int getToken(char *buf, int bufsize)
{
  ...
  return do_pioctl(buf, sizeof(afs_int32), buf, bufsize,
		   VIOCGETTOK, NULL,0);
}
 ...
 */
int authenticateUser(request_rec *r, char *defaultCell, 
		     int cacheExpiration, char *type)
{
  ...
  if ((user[0] =='\0') || (cell[0] == '\0') || (passwd[0] =='\0')) {
    afslog(15, ("%s: pid:%d No username or password or cell. Unlogging.", 
		module_name, getpid()));
    haveAuth = 0;
    userChanged = 1;
    bzero(lastUser, APACHEAFS_USERNAME_MAX);
  ...
  if (strcmp(user, lastUser) || 
    ...
    userChanged = 1;
    afslog(25, ("%s: pid:%d\tUnlogging user %s from cell%s",
    		module_name, getpid(), lastUser, lastCell));
  ...
  if (rc) {
     ...
     */
    afslog(35, ("%s: pid:%d found user %s's token (expires:%d) in cache",
		module_name, getpid(), user, (getExpiration(tokenbuf)-time(NULL))));

    /* if the user changed then set this token else leave it since it should 
       be set */
    if (userChanged) {
      /* set this token obtained from the local cache */
      afslog(15,("%s:pid:%d\t Setting cached token", module_name, getpid()));
      if(setToken(tokenbuf, rc)) {
	afslog(5, ("%s: pid:%d Failed to set token obtained from cache."
		   "rc:%d errno:%d Token Expiration:%d", module_name, getpid(), 
		   rc, errno, (getExpiration(tokenbuf)-time(NULL))));
#ifdef DEBUG_CACHE
	parseToken(tokenbuf);
#endif
	/* 
	...
	if (errno == EDOM) {
	  ...
	  if(setToken(tokenbuf,rc)) { 
	     ...
	     */
	  sprintf(err_msg,"%s: pid:%d Failed to set cached token."
		  "errno:%d rc:%d", module_name, getpid(), errno, rc);
	  LOG_REASON(err_msg, r->uri, r);
	  goto reqAuth;
	  }
	}
	  ...
	  sprintf(err_msg,"%s: Failed to get token: errno:%d rc:%d",
  ...
  reqAuth:
    ...
    rc = request_Authentication(user, passwd, cell, type, tokenbuf,reason);
    if (rc > 0) { 
      /* we got back a token from weblog */
      /* set the token with setToken */
      if(setToken(tokenbuf, rc)) {
	sprintf(err_msg, "%s: Failed to set token given by weblog. errno:%d", 
		module_name, errno);
	LOG_REASON(err_msg, r->uri, r);
	return SERVER_ERROR;
      }
	...
	return SERVER_ERROR;
      ...
      afslog(15, ("%s: pid:%d\t put user:%s tokens in cache", 
      ...
      sprintf(err_msg, ":%s: AFS authentication failed for %s@%s because %s",
      ...
      sprintf(err_msg, "%s: Error readiong from pipe. errno:%d", module_name, errno);
      LOG_REASON(err_msg, r->uri, r);
      return SERVER_ERROR;
    }
       ...
       */
      sprintf(err_msg, "%s: AFS could not authenticate user %s in cell %s."
...
static int get_cellname_from_path(char *apath, char *cell)
{
  ...
  rc = do_pioctl(NULL, 0, cell, APACHEAFS_CELLNAME_MAX,
		     VIOC_FILE_CELL_NAME, apath, 1);
  if (rc) 
    afslog(30, ("%s: Error getting cell from path %s. errno:%d rc:%d",
		module_name, apath, errno, rc));
  else 
    afslog(30, ("%s: Obtained cell %s from path %s", module_name, cell, apath));
 ...
 */
static int getcellname(request_rec *r, char *buf)
{
  int rc = 0;
 ...
 */
static int geturi(request_rec *r, char *buf)
{
  int rc = 0;
  char *pos;
  char *end;
  ...
  if (pos != NULL) {
	...
	strcat(buf,"/");
	strncat(buf,pos,len);
	afslog(35, ("%s: Getting URI upto second slash buf:%s", module_name, buf));
	pos += (len+1);
	end = strchr(pos,'/');
	if (end == NULL) {
	  break;
 ...
 * string Unknown
 */
static int parseAuthName_int(request_rec *r, char *buf, char *msg)
{
  char *pos;
  char *end;
  int len = 0;
  ...
  if (pos) {
    ...
    if (end == NULL) {
      afslog(0,("%s:Parse error for AUTH_REQUIRED reply - mismatched <", module_name));
 ...
 * <%...> <%...>
 */
static int parseAuthName(request_rec *r, char *buf)
{
  char *pos;
  int rc;
  char msg[1024];
  ...
  while (pos != NULL) {
    ...
    if (rc) {
      strcpy(buf, msg);
      afslog(35, ("%s: Failed to parse Auth Name. buf:%s", module_name, buf));
      return -1;
    }
    strcpy(buf, msg);
    bzero(msg, sizeof(msg));
 ...
 */
int forbToAuthReqd(request_rec *r)
{
  if (haveAuth) {
    return FORBIDDEN;
  }
