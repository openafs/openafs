 ...
 */
static int sendToken(int len, void *buf) 
{
  if(buf == NULL) {
...
static int readFromClient(char *buf)
{
  int n;
  if(buf == NULL) {
    WEBLOGEXIT(NULLARGSERROR);
 ...
 */
static int getNullSepWord(char *buf, char sep, char *retBuf, int start)
{
  int ret=0;
 ...
 */
static int parseBuf(char *buf, char *user, char *pass, char *cell, char *type)
{
  char *temp;
  int start=0, ret = 0;
...
static char *makeString(char *sp)
{
  int len;
  char *new_string;
 ...
 * "AFS ID <user_id>".
 */
static int store_afs_token(unix_id, realm_p, tkt_type, ticket_p, ticket_len,
    ...
    int set_pag;
{
	...
	fprintf(stderr, "weblog: DCE ticket is too long (length %d)."
...
static char *make_string(s_p, length)
    char *s_p;
    int length;
{
 ...
 */
static int decode_asn_time(buf, buflen, utime)
    char *buf;
    int buflen;
    afs_int32 *utime;
{
...
static int decode_reply(buf, buflen, reply_p)
 ...
 */
static int getDFScreds(char *name, char *realm, char *passwd, 
		       afs_uint32 lifetime, char **reason)
{
  ...
  if (!cdir) {
    *reason=makeString("weblog: unable to read or open AFS client "
				"configuration  file");
    return -1;
  }
   ...
   */
  if (!reply_p) {
    *reason=(char *) makeString("weblog: unexpected error in server response; aborted");
    return -1;
  }
  ...
  if (strlen(passwd) + strlen(reply_p->salt) + 1 > BUFSIZ) {
    *reason=(char *) makeString("weblog: unexpectedly long passwd/salt combination");
    return -1;
  }
  ...
  /* Destroy the password. */
   ...
   if (code) {
     *reason=(char *) makeString("-- unable to decrypt reply from the DCE KDC");
     return -1;
   }
   ...
   if (code || kdcrep.nonce != nonce) {
     *reason=(char *)makeString("weblog: DCE authentication failed -- "
   ...
   if (code) {
     *reason=(char *)makeString("weblog -- getDCEcreds:failed to store tickets");
     return -1;
   }
   return 0;
}
...
/* All the constant sizes for these arrays are taken from the code for klog */
  ...
  while (!shutdown) {    
    ...
    if(!authtype) {
      ...
      sprintf(reason, "weblog: Unknown Authentication type:%s. AFS-DFS login " 
	      "may not be enabled - check compile flags for ENABLE_DCE_DLOG", type);
      goto reply_failure;
    }
	...
	code=ka_UserAuthenticateGeneral
	  (
	   KA_USERAUTH_VERSION+0, name, NULL, cell, 
	   passwd, lifetime, &password_expires,0,&reason
	   );
      }
#ifdef ENABLE_DCE_DLOG
      else if (authtype == 2) {
...
#ifdef DEBUG
	if (!Silent) {
	  fprintf(stderr,"weblog:Unable to authenticate to AFS because "
		  "%s\n", reason);
	}
#endif
	goto reply_failure;
...
#ifdef DEBUG
	fprintf(stderr,"WEBLOG:After ka_UserAuthenticateGeneral GroupCheck\n");
	printGroups();
#endif
	/* get just the ONE token for this PAG from cache manager */
	i=0;
	memcpy((void *)&tbuffer[0], (void *)&i, sizeof(afs_int32));
	code = do_pioctl(tbuffer, sizeof(afs_int32), tbuffer, sizeof(tbuffer), 
			 VIOCGETTOK, NULL,0);
	if(code) {
	  fprintf(stderr, "weblog: failed to get token:%d\n", code);
	  strcpy(reason, "FAILED TO GET TOKEN FROM CACHE MANAGER\n");
	}
	else {
	  gotToken = 1;
	  ...
	  if (expires < 0 ) {
	    ...
	    if (cacheExpiration == 0) {
	      weblog_login_store(name, cell, cksum, &tbuffer[0],sizeof(tbuffer),expires); 
	    }
	    else {
	      testExpires = cacheExpiration+time(NULL);
	      weblog_login_store(name, cell, cksum, &tbuffer[0],sizeof(tbuffer), 
