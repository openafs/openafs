  ...
  afs_int32 *aothers)
{
	...
	if (inServer) {
	    ...
	    memmove((void *) &temp, (const void *) th->h_addr, sizeof(afs_int32));
	    UNLOCK_GLOBAL_MUTEX
    ...
    if (counter	< MAXSERVERS) *aothers++ = 0;    /* null terminate if room */
    return 0;
}
  ...
  struct ubik_client **aclient)
{
	...
	UNLOCK_UBIK_CLIENT((*aclient))
#ifdef AFS_PTHREAD_ENV
	if (pthread_mutex_destroy(&((*aclient)->cm))) return UMUTEXDESTROY;
#endif
    } else {
    	tc = (struct ubik_client *) malloc(sizeof(struct ubik_client));
    }
    if (tc == NULL) return UNOMEM;
	...
	if (serverconns[i] == (struct rx_connection *) 0) break;
	count++;
    }
	...
	for (count=0; ;count++) { /*s*/
		...
		if (newHost) {
		    ...
		    */
			...
			if (!thisHost) break;
			if (thisHost == newHost) {
			    if (chaseCount++ > 2) break; /* avoid loop asking */
	    ...
	    rcode = (*aproc)(tc, p1,  p2,  p3,  p4,  p5,  p6,  p7,  p8,  p9,
	                     p10, p11, p12, p13, p14, p15, p16);
	    if ( aclient->initializationState != origLevel) {	
		/* somebody did a ubik_ClientInit */
		if ( rcode ) goto restart; /* call failed */
		else    goto done;    /* call suceeded */
	    }
 ...
 */
static int try_GetSyncSite(register struct ubik_client *aclient, afs_int32 apos) {
    struct rx_peer *rxp;
    afs_int32 code;
    int i;
