     ...
     char *s;
{
	...
	*bs = 1;			/* not free anymore */
	...
	if (dbwrite (ut, a, (char *)&b, sizeof(b.h))) return BUDB_IO;
    }
...
afs_int32 FreeStructure (ut, type, sa)
  struct ubik_trans *ut;
...
{   struct blockHeader bh;		/* header of containing block */
    ...
    a = BlockBase(sa);
    if (dbread(ut, a, (char *)&bh, sizeof(bh))) return BUDB_IO;
    if (type != bh.type) db_panic ("block and structure of different types");
