...
void print_msg (text, msg, length)
  char *text;
  afs_int32  msg[];
  int	length;
{   int i;
    ...
    cksum = 0;
    for (i=0; i<length; i++) cksum += *(i + msgtext);
    printf (" (%4x)\n", cksum&0xffff);
}
...
{   afs_uint32 temp;
    ...
    for (temp = orig[0] ^ chgd[0]; temp; temp >>= 1) diff += temp&1;
    for (temp = orig[1] ^ chgd[1]; temp; temp >>= 1) diff += temp&1;
    printf ("%.8x %.8x  (%d)\n", chgd[0], chgd[1], diff);
    number_diff++;
    total_diff += diff;
  ...
  char *argv[];
{
    ...
    if (for_time)
	for (i=0; i<iterations; i++) fc_ecb_encrypt (e,d,schedule, 1);
    else if (for_time_key)
	for (i=0; i<iterations; i++) fc_keysched (&key, schedule);
    else if (for_cbc) {
	afs_int32 msg[10];
	afs_int32 out[10];
	afs_int32 dec[10];
	...
	int rounds;
#ifndef TCRYPT
	printf ("Number of rounds is fixed at %d; try recompiling w/ -DTCRYPT=1\n", ROUNDS);
#endif
	e[0] = 0x11111111;
	e[1] = 0xaaaaaaaa;
	bcopy ("abcdefgh", &key, sizeof(struct ktc_encryptionKey));
