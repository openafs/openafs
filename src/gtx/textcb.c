...
{ /*gator_textcb_Init*/
    ...
    if (initd) {
      fprintf(stderr, "[%s] Initialization routine called multiple times!!\n", rn);
      return(0);
    }
    else
      initd = 1;
...
struct gator_textcb_hdr *gator_textcb_Create(a_maxEntriesStored, a_maxCharsPerEntry)
...
{ /*gator_textcb_Create*/
    ...
    /*
      ...
      * line buffer.
      */
    numBuffBytes = bytesToAllocate = a_maxEntriesStored *
      (a_maxCharsPerEntry + 1);
    ...
    if (gator_textcb_debug)
      fprintf(stderr, "[%s] Allocating %d bytes for the text circular buffer header\n", rn, bytesToAllocate);
    newHdr = (struct gator_textcb_hdr *) malloc(bytesToAllocate);
    if (newHdr == (struct gator_textcb_hdr *)0) {
      fprintf(stderr, "[%s] Can't allocate %d bytes for text circular buffer header; errno is %d\n", rn, bytesToAllocate, errno);
      free(newBuff);
      free(blankLine);
      free(newEntries);
      return((struct gator_textcb_hdr *)0);
      ...
      if (gator_textcb_debug)
	fprintf(stderr, "[%s] Text circular buffer header allocated at 0x%x\n", rn, newHdr);
...
static struct gator_textcb_entry *bumpEntry(a_cbhdr)
...
{ /*bumpEntry*/
     ...
     */
    if (gator_textcb_debug)
	  fprintf(stderr, "[%s]: Bumping entry for circular buffer at 0x%x; current values: currEnt=%d (idx %d), oldestEnt=%d (idx %d), maxEntriesStored=%d\n", rn, a_cbhdr, a_cbhdr->currEnt, a_cbhdr->currEntIdx, a_cbhdr->oldestEnt, a_cbhdr->oldestEntIdx, a_cbhdr->maxEntriesStored);
    ...
    if (a_cbhdr->currEnt >= a_cbhdr->maxEntriesStored) {
      if (gator_textcb_debug)
	fprintf(stderr, "[%s]: Advancing oldest entry number & index; was entry %d, index %d, now ", rn, a_cbhdr->oldestEnt, a_cbhdr->oldestEntIdx);
      a_cbhdr->oldestEnt++;
...
int gator_textcb_Write(a_cbhdr, a_textToWrite, a_numChars, a_highlight, a_skip)
    struct gator_textcb_hdr *a_cbhdr;
    char *a_textToWrite;
    int a_numChars;
    int a_highlight;
...
{ /*gator_textcb_Write*/
    ...
    if (a_cbhdr == (struct gator_textcb_hdr *)0) {
      fprintf(stderr, "[%s]: Null pointer passed in for circ buff header!!  Aborting write operation.\n", rn);
      return(-1);
    }
	...
	*/
      if (gator_textcb_debug)
	fprintf(stderr, "[%s]: Top of write loop: %d char(s) left to write.\n", rn, a_numChars);
	...
	/*
	  ...
	  */
	if (gator_textcb_debug)
	  fprintf(stderr, "[%s]: Entry %d at index %d full, advancing to next one.\n", rn, a_cbhdr->currEnt, curr_ent_idx);	
	curr_ent = bumpEntry(a_cbhdr);
	curr_ent_idx = a_cbhdr->currEntIdx;
	if (gator_textcb_debug)
	  fprintf(stderr, "[%s] New CB entry info: currEnt=%d (idx %d), oldestEnt=%d (idx %d), curr entry ptr is 0x%x\n", rn, a_cbhdr->currEnt, a_cbhdr->currEntIdx, a_cbhdr->oldestEnt, a_cbhdr->oldestEntIdx, curr_ent);
      } /*Bump current entry*/
	...
	chars_to_copy = a_numChars;
      ...
      if (gator_textcb_debug)
	fprintf(stderr, "[%s]: Copying %d char(s) into current entry at 0x%x (entry buffer starts at 0x%x)\n", rn, chars_to_copy, dest, curr_ent->textp);
	...
	*/
	...
	/*
	  ...
	  */
	if (gator_textcb_debug)
	  fprintf(stderr, "[%s]: Highlight mismatch, recording inversion at char loc %d\n", rn, curr_ent->charsUsed);
	if (curr_ent->numInversions < GATOR_TEXTCB_MAXINVERSIONS) {
	  effective_highlight = a_highlight;
	  curr_ent->inversion[curr_ent->numInversions] = curr_ent->charsUsed;
	...
	*/
	...
	if (gator_textcb_debug)
	  fprintf(stderr, "[%s] Not skipping, we're already on a fresh entry\n", rn);
...
int gator_textcb_BlankLine(a_cbhdr, a_numBlanks)
    struct gator_textcb_hdr *a_cbhdr;
    int a_numBlanks;
...
int gator_textcb_Delete(a_cbhdr)
    struct gator_textcb_hdr *a_cbhdr;
...
{ /*gator_textcb_Delete*/
    ...
    /*
      ...
      */
    if (gator_textcb_debug)
      fprintf(stderr, "[%s]: Freeing text buffer proper at 0x%x (%d bytes)\n",
    ...
    if (gator_textcb_debug)
      fprintf(stderr, "[%s]: Freeing text entry array at 0x%x (%d bytes)\n",
	      rn, a_cbhdr->entry,
	      (a_cbhdr->maxEntriesStored * sizeof(struct gator_textcb_entry)));
    free(a_cbhdr->entry);
