...
#include "k5s_cast.h"
#include "k5s_cast_sboxes.h"

/* Macros to access 8-bit bytes out of a 32-bit word */
#define U8a(x) ( (cast_u8) (x>>24) )
#define U8b(x) ( (cast_u8) ((x>>16)&255) )
#define U8c(x) ( (cast_u8) ((x>>8)&255) )
#define U8d(x) ( (cast_u8) ((x)&255) )
...
cast_u32 t, l, r;
	...
	l = ((cast_u32)inblock[0] << 24) | ((cast_u32)inblock[1] << 16) |
	 ((cast_u32)inblock[2] << 8) | (cast_u32)inblock[3];
	r = ((cast_u32)inblock[4] << 24) | ((cast_u32)inblock[5] << 16) |
	 ((cast_u32)inblock[6] << 8) | (cast_u32)inblock[7];
	/* Do the work */
	F1(l, r,  0);
	F2(r, l,  1);
	F3(l, r,  2);
...
cast_u32 t, l, r;
	...
	/* Get inblock into l,r */
	r = ((cast_u32)inblock[0] << 24) | ((cast_u32)inblock[1] << 16) |
	 ((cast_u32)inblock[2] << 8) | (cast_u32)inblock[3];
	l = ((cast_u32)inblock[4] << 24) | ((cast_u32)inblock[5] << 16) |
	 ((cast_u32)inblock[6] << 8) | (cast_u32)inblock[7];
	/* Do the work */
	/* Only do full 16 rounds if key length > 80 bits */
	if (key->rounds > 12) {
...
{
  cast_u32 t[4], z[4], x[4];
  int i;
  
  /* gcc 4.0 thinks t and z not initialized */
#if 0
  memset(&t, 0, sizeof(t));
  memset(&z, 0, sizeof(z));
#endif
  ...
  for (i = 0; i < 4; i++) {
    x[i] = 0;
    if ((i*4+0) < keybytes) x[i] = (cast_u32)rawkey[i*4+0] << 24;
    if ((i*4+1) < keybytes) x[i] |= (cast_u32)rawkey[i*4+1] << 16;
    if ((i*4+2) < keybytes) x[i] |= (cast_u32)rawkey[i*4+2] << 8;
    if ((i*4+3) < keybytes) x[i] |= (cast_u32)rawkey[i*4+3];
  }
