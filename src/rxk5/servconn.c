...
static int
k4_get_cred(char *service,
    char *instancel
    char *realm,
    union k45_cred *cred,
    struct rx_krb_cred *rkcred)
{
...
Out:
    if (code) afs_com_err("k5_get_cred", code, when);
#ifdef USING_SHISHI
    if (hint->server)
	free(hint->server);
    if (hint->serverrealm)
...
int getservconn(struct servparm *sps,
...
int
setservhost(struct servparm *sps,
    char *cp,
    char *word)
{
    int f;
...
int
setservkrbname(struct servparm *sps,
    char *cp,
    char *word)
{
    int f;
