...
int DirOK (file)
    ...
    ** not exists for pages between MAXPAGES and BIGMAXPAGES */
	...
	if ( (i < MAXPAGES) && ((count & 0xff) != (dhp->alloMap[i] & 0xff)) ) {
	   printf("Header alloMap count doesn't match count in freebitmap for page %d.\n", i);
	   DRelease(pp, 0);
	   DRelease(dhp, 0);
	   return 0;
	}
	...
	for (entry = ntohs(dhp->hashTable[i]); entry; entry = ne) {
	    ...
	    /* Check that if this is entry 13 (the 1st entry), then name must be "." */
		  ...
		  printf("Dir entry %x, index 13 has name '%s' should be '.'\n",
			 ep, ep->name);
		  DRelease(ep, 0);
		  DRelease(dhp, 0);
		  return 0;
	    ...
	    /* Check that if this is entry 14 (the 2nd entry), then name must be ".." */
		  ...
		  printf("Dir entry %x, index 14 has name '%s' should be '..'\n",
			 ep, ep->name);
		  DRelease(ep, 0);
		  DRelease(dhp, 0);
		  return 0;
    ...
    if (!havedot || !havedotdot) {
       printf("Directory entry '.' or '..' does not exist or is in the wrong index.\n");
       DRelease(dhp, 0);
       return 0;
    }
    ...
    afs_int32 vn, vu, pvn, pvu;
{
    ...
    for(i=0; i<NHASHENT; i++) {
	    ...
	    DErrno = 0;
            ...
            if (!ep) {
		if (DErrno) {
		    printf("can't continue down hash chain (entry %d, errno %d)\n",
			   entry, DErrno);
		    DRelease(dhp, 0);
		    return DErrno;
		}
	    ...
	    if ((strcmp(tp,".") != 0) && (strcmp(tp,"..") != 0)) {
	       ...
	       if (code) {
		  printf("Create of %s returned code %d, skipping to next hash chain.\n", tname, code);
		  DRelease(ep, 0);
		  break;
	       }
	    }
