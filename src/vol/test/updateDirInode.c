...
{
	assert(name);
	printf("Usage is %s -p <partition name> -v <volume name> -dirInode <directory inode> -f <directoryFileName> -verbose\n", name);
	exit(1);
}
...
int pageNo;
{
    ...
    if ( fscanf(fp, "freebitmap :%x %x %x %x %x %x %x %x", 
		&freebit[0], &freebit[1], &freebit[2], &freebit[3],
		&freebit[4], &freebit[5], &freebit[6], &freebit[7]) != 8 )
    {
    ...
    for ( ; start < EPP; start++ )
    {
	if ( fscanf(fp, "ent %d: f=%d l=%d n=%d vn=%d vu=%d name:%s\n",
	    &ent, &f, &l, &n, &vnode, &unique, dirName) != 7 )
        {
