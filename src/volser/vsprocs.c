	       ...
	       afs_int32 atoserver, afs_int32 atopart, int flags)
{
    ...
    code = AFSVolTransCreate(fromconn, afromvol, afrompart, ITBusy, &tmp);
    fromtid = tmp;
    ...
    volid = afromvol;
    tmp = totid;
    code = AFSVolTransCreate(toconn, volid, atopart, ITOffline, &tmp);
    totid = tmp;
    if (!code) {
    ...
    tmp = fromtid;
    code =
	AFSVolTransCreate(fromconn, backupId, afrompart, ITOffline, &tmp);
    fromtid = tmp;
    VDONE;
    if (!code) {
  ...
  mfail:
	...
	if (volid && toconn) {
	    ...
	    tmp = totid;
	    code =
		AFSVolTransCreate(toconn, volid, atopart, ITOffline, &tmp);
	    totid = tmp;
	       ...
	       afs_int32 atovolid, int flags)
{
    ...
    cloneFromDate = 0;
    tmp = totid;
    code = AFSVolTransCreate(toconn, newVol, atopart, ITOffline, &tmp);
    totid = tmp;
    if (!code) {
	if ((flags & RV_CPINCR)) {
