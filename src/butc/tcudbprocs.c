...
afs_int32 CreateDBDump(dumpEntryPtr)
 ...
 */
afs_int32 GetDBTape(taskId, expires, tapeInfoPtr, dumpid, sequence, queryFlag, wroteLabel)
    afs_int32   taskId;
    Date   expires;
    ...
    int    *wroteLabel;
{
    ...
    *wroteLabel = 0;
	...
	{
	    code = PromptForTape(SAVEDBOPCODE, tapeName, dumpid, taskId, tapecount);
	...
	else
	{
	    ...
	    if ( dump_namecheck &&
		 strcmp(oldTapeLabel.AFSName,"") && !databaseTape(oldTapeLabel.AFSName) )
	    {
	        char gotName[BU_MAXTAPELEN+32];

		LABELNAME(gotName, &oldTapeLabel);
	        TLog(taskId, "This tape %s must be a database tape or NULL tape\n", gotName);
		...
		if (oldTapeLabel.dumpid && (oldTapeLabel.dumpid == lastDump.id)) {
		    ErrorLog(0, taskId, 0, 0, 
			     "Warning: Overwriting most recent dump %s (DumpID %u)\n",
	...
	if (code)
	{
	    TapeLog(0, taskId, code, tapeInfoPtr->error, "Can't label tape\n");
	    goto getNewTape;
	}
...
error_exit:
	...
	if (dumpEntry.id != 0)
	{
	    ...
	    if (i)
	    {
		    ...
		    TapeLog(0, taskId, i, 0, "Dump id %d not added to database\n", dumpEntry.id);
	    }
	    else
	    {
	        i = addTapesToDb(taskId);
    ...
    afs_uint32               dumpid;
{
        ...
        if ( charList.charListT_len == 0 )
	{
		...
		if (code) {
		    ErrorLog(0, taskId, code, 0, "Can't pthread_attr_init Keep-alive process\n");
		    ERROR_EXIT(code);
		}

		code = pthread_attr_setdetachstate(&tattr, PTHREAD_CREATE_DETACHED);
		if (code) {
		    ErrorLog(0, taskId, code, 0, "Can't pthread_attr_setdetachstate Keep-alive process\n");
		    ERROR_EXIT(code);
		}
...
error_exit:
    if (ctPtr) 
    {
        if (ctPtr->textStream) fclose(ctPtr->textStream);
	if (tlock) bc_UnlockText(ctPtr);
	free(ctPtr);
    }
    return(code);
}
     ...
     struct structDumpHeader *nextHeader;
{
    ...
    if (!readBuffer) ERROR_EXIT(TC_NOMEMORY);
	...
	code = getTapeData(tapeInfo, rstTapeInfoPtr, readBuffer, transferSize);
	if (code) ERROR_EXIT(code);
