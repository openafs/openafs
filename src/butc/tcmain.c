     ...
     afs_int32 *number;
{
    ...
    count = sscanf(numstring, "%f%c%s", (unsigned char *)&total, &cunit, rest);
    if ((count > 2) || (count <= 0))
       return -1;
    if (count == 1)
       cunit = 'B';        /* bytes */
     ...
     afs_int32 portOffset;
{
	...
	if (count == 4) {
	    ...
	    if ( atocl(tfmsize, 'B', &fmSize) ) {
	        fprintf(stderr, "tapeconfig: File-mark size parse error in: %s\n", line);
		ERROR_EXIT(-1);
	    }
	} else {
    ...
    afs_int32 port;
{
	...
	{
	    if (CONF_XBSA) {
	        printf("Warning: The %s parameter is ignored with a Backup Service\n", cmd);
		continue;
	    }
	...
	{
	    if (!CONF_XBSA) {
	        printf("Warning: The %s parameter is ignored with a tape drive\n", cmd);
		continue;
	    }
	    ...
	    printf("XBSA type is %s\n", 
		   ((xbsaType == XBSA_SERVER_TYPE_UNKNOWN)?"Unknown":value));
	}
	...
	else if (!strcmp(cmd,"PASSWORD"))  /* This or PASSFILE required for XBSA */
	{
	    ...
	    if (xbsaSecToken) {
	        printf("Warning: The %s parameter is ignored. Already read password\n", cmd);
		continue;
	    }
	...
	else if (!strcmp(cmd,"PASSFILE"))  /* This or PASSWORD required for XBSA */
	{
	    ...
	    if (!pwdFile) {
	       printf("Configuration file error, cannot open password file %s\n", value);
	       ERROR_EXIT(1);
	    }
	    xbsaSecToken = malloc(LINESIZE);
	    if (!fscanf(pwdFile, "%s", xbsaSecToken)) {
	       printf("Configuration file error, cannot read password file %s\n", value);
	       ERROR_EXIT(1);
	    }
	...
	else if (!strcmp(cmd,"MAXPASS"))
	{
	    ...
	    if (maxpass < PASSESMIN) maxpass = PASSESMIN;
	    if (maxpass > PASSESMAX) maxpass = PASSESMAX;
	    printf("MAXPASS is %d\n", maxpass);
	}
	...
	else if (!strcmp(cmd,"STATUS"))
	{
	    ...
	    if (statusSize < MINSTATUS) statusSize = MINSTATUS;
	    if (statusSize > MAXSTATUS) statusSize = MAXSTATUS;
	}
    ...
    if (statusSize) {
       ...
       if (statusSize < 0) statusSize = 0x7fffffff; /*max size*/
  ...
  error_exit:
    ...
    if (!code && CONF_XBSA) {
	  ...
	  printf("Configuration file error, the TYPE parameter must be specified, or\n");
	  printf("an entry must exist in %s for port %d\n", tapeConfigFile, port);
	  code = 1;
       }
       if (!adsmServerName) {
	  printf("Configuration file error, the SERVER parameter must be specified\n");
	  code = 1;
       }
       if (!xbsaSecToken) {
	  printf("Configuration file error, the PASSWORD or PASSFILE parameter must be specified\n");
	  code = 1;
       }
    }
#endif /*xbsa*/
	...
	*/
    ...
    if (CONF_XBSA) {
	  ...
	  printf("Force XBSA multiple server support\n");
       ...
       if (rc != XBSA_SUCCESS) return(1);
   }
#endif /*xbsa*/

    /* cell switch */
    if (as->parms[2].items) strncpy(cellName, as->parms[2].items->data, sizeof(cellName));
    else                    cellName[0] = '\0';
    ...
    if (code)
    {
        TapeLog(0, 0, code, 0, "Can't pthread_attr_init database monitor task");
	exit(1);
    }
    code = pthread_attr_setdetachstate(&tattr, PTHREAD_CREATE_DETACHED);
    if (code)
    {
        TapeLog(0, 0, code, 0, "Can't pthread_attr_setdetachstate database monitor task");
	exit(1);
    }
    AFS_SIGSET_CLEAR();
...
char **argv; 
{
    ...
    strcompose(eFile, AFSDIR_PATH_MAX, AFSDIR_SERVER_BACKUP_DIRPATH, "/", TE_PREFIX, NULL);
    strcompose(lFile, AFSDIR_PATH_MAX, AFSDIR_SERVER_BACKUP_DIRPATH, "/", TL_PREFIX, NULL);
    strcompose(pFile, AFSDIR_PATH_MAX, AFSDIR_SERVER_BACKUP_DIRPATH, "/", CFG_PREFIX, NULL);
