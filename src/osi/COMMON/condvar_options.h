...
#ifndef _OSI_COMMON_CONDVAR_OPTIONS_H
#define _OSI_COMMON_CONDVAR_OPTIONS_H 1
...
#define osi_condvar_options_Destroy(opt)

#define osi_condvar_options_Copy(dst, src) \
    osi_Macro_Begin \
        if (src) { \
            osi_mem_copy((dst), (src), sizeof(osi_condvar_options_t)); \
        } else { \
            osi_condvar_options_Init(dst); \
        } \
    osi_Macro_End

osi_extern void osi_condvar_options_Set(osi_condvar_options_t * opt,
					osi_condvar_options_param_t param,
					long val);

#endif /* _OSI_COMMON_CONDVAR_OPTIONS_H */
