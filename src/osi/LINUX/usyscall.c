...
osi_linux_proc_syscall(long call, long p1, long p2, long p3, long p4, int * rval);

osi_static osi_result
osi_linux_proc_syscall_open(osi_fd_t *);
osi_static osi_result
osi_linux_proc_syscall_close(osi_fd_t);
...
osi_syscall_mux_call(long call, long p1, long p2, long p3, long p4, int * rval)
{
	...
	*rval = code = syscall(AFS_SYSCALL, call, p1, p2, p3, p4);
	...
	res = osi_linux_proc_syscall(call, p1, p2, p3, p4, rval);
    } else {
	res = OSI_ERROR_NOSYS;
    }
...
osi_static osi_result
osi_linux_proc_syscall(long call, 
		       ...
		       int * rval)
{
	...
	res = osi_linux_proc_syscall_open(&fd);
    ...
    *rval = code = ioctl(fd, 
    ...
    if (!osi_syscall_state.ioctl_persistent) {
	(void)osi_linux_proc_syscall_close(fd);
    }
...
osi_static osi_result
osi_linux_proc_syscall_open(osi_fd_t * fd_out)
{
    osi_result res = OSI_OK;
    osi_fd_t fd;

    fd = open(PROC_SYSCALL_FNAME, O_RDWR);
...
osi_static osi_result
osi_linux_proc_syscall_close(osi_fd_t fd)
{
...
osi_syscall_user_os_PkgInit(void)
{
 ...
 probe:
    code = osi_linux_proc_syscall_open(&fd);
	...
	osi_linux_proc_syscall_close(fd);
    }
