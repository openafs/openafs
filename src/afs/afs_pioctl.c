...
DECL_PIOCTL(PSetAcl)
{
    ...
    do {
	tconn = afs_Conn(&avc->fid, areq, SHARED_LOCK, &rxconn);
	if (tconn) {
	    XSTATS_START_TIME(AFS_STATS_FS_RPCIDX_STOREACL);
	    RX_AFS_GUNLOCK();
	    code =
		RXAFS_StoreACL(rxconn, (struct AFSFid *)&avc->fid.Fid,
			       &acl, &OutStatus, &tsync);
	    RX_AFS_GLOCK();
	    XSTATS_END_TIME;
	} else
	    code = -1;
	     ...
	     (tconn, rxconn, code, &avc->fid, areq, AFS_STATS_FS_RPCIDX_STOREACL,
	      SHARED_LOCK, NULL));
...
DECL_PIOCTL(PGetVolumeStatus)
{
	...
	if (tc) {
	    ...
	    RX_AFS_GUNLOCK();
	    code =
		RXAFS_GetVolumeStatus(rxconn, avc->fid.Fid.Volume, &volstat,
	    ...
	    code = -1;
	     ...
	     (tc, rxconn, code, &avc->fid, areq, AFS_STATS_FS_RPCIDX_GETVOLUMESTATUS,
	      SHARED_LOCK, NULL));
...
DECL_PIOCTL(PSetVolumeStatus)
{
	...
	if (tc) {
	    ...
	    RX_AFS_GUNLOCK();
	    code =
		RXAFS_SetVolumeStatus(rxconn, avc->fid.Fid.Volume, &storeStat,
	    ...
	    code = -1;
	     ...
	     (tc, rxconn, code, &avc->fid, areq, AFS_STATS_FS_RPCIDX_SETVOLUMESTATUS,
	      SHARED_LOCK, NULL));
...
DECL_PIOCTL(PRemoveMount)
{
	...
	if (tc) {
	    ...
	    RX_AFS_GUNLOCK();
	    code =
		RXAFS_RemoveFile(rxconn, (struct AFSFid *)&avc->fid.Fid, bufp,
				 &OutDirStatus, &tsync);
	    RX_AFS_GLOCK();
	    XSTATS_END_TIME;
	} else
	    code = -1;
	     ...
	     (tc, rxconn, code, &avc->fid, areq, AFS_STATS_FS_RPCIDX_REMOVEFILE,
	      SHARED_LOCK, NULL));
...
DECL_PIOCTL(PPrefetchFromTape)
{
    ...
    do {
	tc = afs_Conn(&tvc->fid, areq, SHARED_LOCK, &rxconn);
	if (tc) {
	    ...
	    code = -1;
	     ...
	     (tc, rxconn, code, &tvc->fid, areq, AFS_STATS_FS_RPCIDX_RESIDENCYRPCS,
	      SHARED_LOCK, NULL));
