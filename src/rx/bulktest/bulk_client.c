    ...
    char *data;
{
	...
	error = ((work->store? StoreFile:FetchFile)(call, work->verbose, work->local, work->remote, &length));
	error = rx_EndCall(call, error);

	clock_NewTime();
	clock_GetTime(&endTime);
	msec = clock_ElapsedTime(&startTime, &endTime);
	if (work->verbose && !error) printf("%s: %s %s, %d bytes in %d msec, %d bps\n", work->host, work->store? "stored" : "fetched", work->store? work->local: work->remote, length, msec, length*1000/msec);
	else if (error) printf("%s: %s of %s failed: error %d\n", work->host, work->store? "store":"fetch", work->store? work->local:work->remote, error);
    }
    async_nProcs--;
    ...
    char *file;
{
    ...
    char *name;
    PROCESS pid;
    struct async_work *work = (struct async_work *) malloc(sizeof(struct async_work));
    work->host = host;
    work->conn = conn;
    work->store = store;
    work->count = count;
...
/*   sprintf(tempfile, "/usr/tmp/%s.%s", myHostName, name);*/
    ...
    async_nProcs += 1;
    LWP_CreateProcess(async_BulkProc, 3000, RX_PROCESS_PRIORITY, (void *) work, "bulk", &pid);
}

long FetchFile(call, verbose, localFile, remoteFile, length_ptr)
    struct rx_call *call;
    int verbose;
    ...
    long *length_ptr;
{
    ...
    if (StartBULK_FetchFile(call, verbose, remoteFile)) return BULK_ERROR;
    fd = open(localFile, O_CREAT|O_TRUNC|O_WRONLY, 0666);
    if (fd < 0 || fstat(fd, &status) < 0) {
	fprintf("Could not create %s\n", localFile);
	error = BULK_ERROR;
    }
    else if (bulk_ReceiveFile(fd, call, &status)) error = BULK_ERROR;
...
long StoreFile(call, verbose, localFile, remoteFile, length_ptr)
    struct rx_call *call;
    int verbose;
    char *localFile, *remoteFile;
    long *length_ptr;
...
char **argv;
{
	...
	while (**argv == '-') {
		...
		conn = rx_NewConnection(host, BULK_SERVER_PORT, BULK_SERVICE_ID, null_securityObject, BULK_NULL);
	    }
	    else if (strcmp(*argv, "-f") == 0) store = 0;
	    else if (strcmp(*argv, "-s") == 0) store = 1;
	    else if (strcmp(*argv, "-v") == 0) verbose = 1;
	    else if (strcmp(*argv, "-c") == 0) (count = atoi(*++argv)), argc--;
	    else {
    ...
    if (verbose) fprintf(stderr, "All transfers done\n");
    rx_PrintStats(stdout);
