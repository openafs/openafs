...
char **argv;
{
	...
	fprintf(stderr, "ka_util: Ubik Version number changed during execution.\n");
...
int offset;
{
    ...
    if (lseek (dbase_fd, offset+HDRSIZE+sizeof(struct kaheader), L_SET) < 0)
      return -1;
    ...
    fprintf(dfp, "%s%s%s %d %d %d %d %d %d %d ",
	    dbentry.userID.name,
