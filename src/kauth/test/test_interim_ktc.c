...
static void PrintPrincipal (stream, p)
  FILE *stream;
  struct ktc_principal *p;
{
    if (!verbose) return;
    fprintf (stream, "%s", p->name);
    if (strlen(p->instance)) fprintf (stream, ".%s", p->instance);
    if (strlen(p->cell) && (strcmp (p->cell, ka_LocalCell()) != 0))
	fprintf (stream, "@%s", p->cell);
}

static void PrintAuthentication (stream, s, t, c)
  FILE *stream;
  ...
  struct ktc_token *t;
{
    ...
    fprintf (stream, "  good for %d seconds till %s",
	     lifetime, bob);
    ...
    else if (now+1 < t->startTime) fprintf (stream, " [ahead]\n");
    else if (now >= t->endTime+KTC_TIME_UNCERTAINTY)
	fprintf (stream, " [EXPIRED]\n");
    else if (now >= t->endTime) fprintf (stream, " [behind]\n");
    else fprintf (stream, "\n");
...
static int CheckUnixUID (server, token, client)
    ...
    struct ktc_principal *client;
{
    ...
    lifetime = (unsigned long)ntoken.endTime - (unsigned long)ntoken.startTime;
    if ((lifetime & 1) == 1) {
	com_err (whoami, code,
		   "GetToken returned even lifetime (%d)", lifetime);
	return 1;
    }
    return 0;
}

static int CheckAFSId (server, token, client)
    struct ktc_principal *server;
    struct ktc_token *token;
    struct ktc_principal *client;
{
	...
	(strlen (nclient.name) < 8) ||
	(atoi (nclient.name + 7) != viceId) ||
	strlen (nclient.instance)) {
	fprintf (stderr, "GetToken returned bad client: '");
	PrintPrincipal (stderr, &nclient);
  ...
  struct ktc_principal *server;
{
...
#if DB_CELLS
    fprintf(stderr, "U_CellSetLocalTokens: using isPrimary=%d, cellName='%s'\n",
            primaryFlag, cellID);
#endif DB_CELLS
    ...
    if ((strcmp (client.name, "AFS ID 123") != 0) ||
	strlen (client.instance) ||
  ...
  char *pathname;
{
    ...
    if (verbose > 1) printf ("old acl for %s is %s\n", pathname, space);
    al = ParseAcl(space);
	...
	for (ae=al->pluslist; ae; ae = ae->next) {
		...
		!(ae->rights & (PRSFS_INSERT | PRSFS_DELETE |
				PRSFS_WRITE | PRSFS_LOCK |
				PRSFS_ADMINISTER))) ae->rights = 0;
	}
...
static int TryAuthenticating (name, password, viceId, cell)
  char *name;
  char *password;
  long  viceId;
  char *cell;
{
    ...
    if (code) {
	com_err (whoami, code,
		 "so couldn't get %s's afs token in %s", name, cell);
	return code;
    }
	...
	if ((strcmp (client.name, tmp) != 0) ||
	    ...
	    PrintPrincipal (stderr, &client);
	    fprintf (stderr,
		     "' should have gotten AFS ID %d for %s@%s\n",
		     viceId, name, cell);
	    return 1;
	}
    }
...
static long CheckAFSTickets ()
{
	...
	else fprintf (stdout, "Using kernel ticket cache\n");
    }
...
#define IsArg(a) (strncmp (arg,a, arglen) == 0)
	if (IsArg("-quiet")) verbose--;
	else if (IsArg("-verbose")) verbose++;
	else if (IsArg("-printtoken")) printToken++;
	else if (IsArg("-remotecell")) strncpy (cell, argv[++i], sizeof(cell));
	else if (IsArg("-testid")) testerId = atoi (argv[++i]);
	else if (IsArg("-tester")) strncpy (tester, argv[++i], sizeof(tester));
	else if (IsArg("-testpassword"))
	    strncpy (testerPassword, argv[++i], sizeof(testerPassword));
	else if (IsArg("-remotetestid")) remoteTesterId = atoi (argv[++i]);
	  ...
	  usage:
	    fprintf (stderr, "Usage is: '%s [-quiet] [-verbose] [-remotecell <cellname>] [-testid <AFS ID>] [-tester <name>] -testpassword <pass> [-remotetestid <AFS ID> -remotetester <name> -remotetestpassword <pass>]\n", whoami);
	    exit (1);
	}
	i++;
    }
    ...
    if (code) com_err (whoami, code, "calling cell config");
  ...
  unused_cell_found:
    unusedCell = i;
    if (verbose) printf ("Using unused cell %s\n", cellNameList[unusedCell]);
	...
	com_err (whoami, code, "should have gotten error calling SetToken with zero ticket length");
	goto failed;
    }
    ...
    if (CheckAFSId (&server, &ntoken, &nclient)) goto failed;
    ntoken.endTime--;
    if (CheckAFSId (&server, &ntoken, &nclient)) goto failed;
    ...
    if (CheckUnixUID (&server, &ntoken, &nclient)) goto failed;
    ...
    if (CheckAuth2 (&server)) goto failed;
