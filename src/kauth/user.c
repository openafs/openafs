  ...
  afs_int32 flags)
{
  ...
  struct ktc_principal server, client;
	...
	if (code) {
	    com_err (whoami, code, "initializing ptserver in cell '%s'", server.cell);
	    return 0;
	}
	len = strlen (name);
	if (instance[0]) len += strlen(instance)+1;
	...
	pr_End();
	if ((code == 0) && (viceId == ANONYMOUSID)) code = PRNOENT;
	if (code) {
  ...
  char **reasonP)
{
	...
	if (reasonP) *reasonP = "root is only authenticated locally";
	return KANOENT;
    }
    code = ka_Init(0);
    if (code) return code;
...
#ifdef AFS_DUX40_ENV
	if (flags & KA_USERAUTH_DOSETPAG) afs_setpag();
#else
#if !defined(UKERNEL) && !defined(AFS_NT40_ENV)
	if (flags & KA_USERAUTH_DOSETPAG) setpag();
#endif
#endif
	if (flags & KA_USERAUTH_DOSETPAG2) dosetpag = 1;
#ifdef AFS_KERBEROS_ENV
	if ((flags & KA_USERAUTH_DOSETPAG) || dosetpag) ktc_newpag();
#endif
	if (lifetime == 0) lifetime = MAXKTCTICKETLIFETIME;
	code = GetTickets (name, instance, realm, &key, lifetime, password_expires, dosetpag);
	if (code == KABADREQUEST) {
	    des_string_to_key(password, &key);
	    code = GetTickets(name, instance, realm, &key, lifetime, password_expires, dosetpag);
	}
...
afs_int32 ka_UserAuthenticate (
  ...
  char **reasonP)
{
    return ka_UserAuthenticateGeneral
	(KA_USERAUTH_VERSION + ((doSetPAG) ? KA_USERAUTH_DOSETPAG : 0), 
	 name, instance, realm, password, /*lifetime*/0, /*spare1,2*/0,0,
	 reasonP);
}

#if !defined(UKERNEL)
afs_int32 ka_UserReadPassword (
  char *prompt,
  char *password,
  int   plen,
  char **reasonP)
...
afs_int32 ka_VerifyUserPassword(
     afs_int32 version,
     char *name,
     char *instance,
     char *realm,
     ...
     char **reasonP)
{
    ...
    return ka_UserAuthenticateGeneral(version | KA_USERAUTH_ONLY_VERIFY,
				      name, instance, realm, password,
				      0, &pwexpires, spare, reasonP);
}
