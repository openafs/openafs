...
main(int argc, char ** argv)
{
    ...
    nhits = 0;
    code = osi_TracePoint_EnableByFilter((osi_trace_gen_id_t)gen_id,
					 argv[2],
					 &nhits);
    if (OSI_RESULT_FAIL(code)) {
	fprintf(stderr, "EnableByFilter failed with code %d (errno %d)\n",
		code, errno);
	ret = -1;
    } else {
	printf("EnableByFilter returned success; nhits=%u\n", nhits);
	ret = 0;
    }
