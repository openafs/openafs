...
main(int argc, char ** argv)
{
    ...
    for (i=1; i <= max_id; i++) {
        (osi_Msg "calling osi_trace_gen_rgy_sys_lookup_I2A(%d)...", i);
        res = osi_trace_gen_rgy_sys_lookup_I2A(i, &addr);
	if (OSI_RESULT_FAIL(res)) {
	    (osi_Msg "FAILED!!!\n");
	} else {
	    (osi_Msg "done\n");
	    (osi_Msg "\tgen_id=%d  pid=%d  programType=%d (%s)\n", 
	     i, addr.pid, addr.programType, osi_ProgramTypeToString(addr.programType));
	}
    }
