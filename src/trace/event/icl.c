		   ...
		   osi_icl_arg_t arg)
{
	...
	res = osi_TracePoint_record_arg_add64(osi_Trace_EventHandle_RecordPtr(handle), 
					      tmp64);
	break;
    ...
    case ICL_TYPE_INT32:
    case ICL_TYPE_LONG:
	res = osi_TracePoint_record_arg_add(osi_Trace_EventHandle_RecordPtr(handle), 
					    arg.int_arg);
	break;
	...
	res = osi_TracePoint_record_arg_add64(osi_Trace_EventHandle_RecordPtr(handle), 
					      (osi_int64)(arg.ptr_arg));
#else
	res = osi_TracePoint_record_arg_add(osi_Trace_EventHandle_RecordPtr(handle), 
					    (osi_register_int)(arg.ptr_arg));
#endif
	break;

    case ICL_TYPE_VENUS_FID:
	(void)osi_TracePoint_record_arg_add(osi_Trace_EventHandle_RecordPtr(handle),
...
osi_result
osi_TraceFunc_iclVarEvent(osi_trace_probe_id_t probe_id,
			  ...
			  osi_va_list args)
{
	...
	types = osi_va_arg(args, osi_register_int);
	res = osi_TracePoint_record_arg_add(osi_Trace_EventHandle_RecordPtr(&handle),
					    types);
	...
	for (i=1; i < nf; i++) {
	    data.int_arg = osi_va_arg(args, osi_register_int);
