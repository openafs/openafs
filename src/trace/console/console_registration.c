...
osi_result
osi_trace_console_register(osi_Trace_rpc_addr4 * addr,
			   osi_Trace_rpc_console_options * options,
			   osi_Trace_rpc_bulkaddrs4 * addrs)
{
    ...
    code = Agent_ConsoleRegisterIP4(conn,
				    &osi_trace_console_uuid,
				    options,
				    addrs->osi_Trace_rpc_bulkaddrs4_len,
				    0,
				    addrs);
    res = (osi_result) code;
...
osi_result
osi_trace_console_unregister(osi_Trace_rpc_addr4 * addr)
{
    osi_result res;
    afs_int32 code;
    struct rx_connection * conn;
    ...
    code = Agent_ConsoleUnregister(conn,
				   &osi_trace_console_uuid);
    res = (osi_result) code;
