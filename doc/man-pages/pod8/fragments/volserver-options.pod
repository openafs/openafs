...
F<{{ afslogsdir }}/VolserLog> file. Provide one of the following values, each
of which produces an increasingly detailed trace: C<0>, C<1>, C<5>, C<25>,
and C<125>. 

=item B<-log>

Records in the {{ afslogsdir }}/VolserLog file the names of all users who
...
F<{{ afslogsdir }}/VolserLog> to F<{{ afslogsdir }}/VolserLog.old> when the volume server is
...
no other logging options are supplied, this will be F<{{ afslogsdir }}/VolserLog>.
